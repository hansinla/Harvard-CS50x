...
$(window).load(function() {
    ...
    $(document.body).keydown(function(event) {
        $("#announcements").html("On the move again...");
...
function chart()
{
    ...
    for (var i = 0; i < shuttle.seats.length; i++)
    {
        ...
        {
            html += "<li>" + shuttle.seats[i].house + ":    " + shuttle.seats[i].name + "</li>";
        }
    }
    html += "</ol>";
    $("#chart").html(html);
...
function dropoff()
{
    ...
    if (noDropOffs == true)
        {
            $("#announcements").html("Nobody on board with this destination.");
        }
}
...
function pickup()
{
    ...
    if (numEmptySeats > 0)
    {
        ...
        {
            var lat = PASSENGERS[j].placemark.getGeometry().getLatitude();
            var lng = PASSENGERS[j].placemark.getGeometry().getLongitude(); 
            var d = shuttle.distance(lat, lng);
            if (d <= 15.0)
            {
                for (var i = 0; i < shuttle.seats.length; i++)
                {
                    if (PASSENGERS[j].house in HOUSES)
                    {
                        if (shuttle.seats[i] == null)
                        {
                            shuttle.seats[i] = PASSENGERS[j];
                            numEmptySeats--;
                            if (numEmptySeats == 0)
                            {
                                $("#announcements").html("No empty seats left.");
                            }
                            var features = earth.getFeatures();
                            features.removeChild(PASSENGERS[j].placemark);
                            PASSENGERS[j].marker.setMap(null);
                            delete PASSENGERS[j];
                            chart();
                            break;
                        }
                    ...
                    {
                        $("#announcements").html("I can not pick up this passenger.");
                        break;
                    }
                }
            }
    ...
    {
        // alert, there are no more empty seats
        $("#announcements").html("No empty seats left.");
